...
export function initPositional() {
  ...
  function drawAddition() {
    ...
    const pe = Array.from(
      { length: dShow },
      (_, d) => +peValue(currentPos, d).toFixed(2),
    );
    ...
    renderMatrix(row, [embedding], `Embedding (pos=${currentPos})`, '#5C6BC0', {
      cellSize: 52,
    });
    const plus = document.createElement('div');
    plus.style.cssText =
      'display:flex;align-items:center;font-size:1.5rem;color:#888;margin:0 6px;';
    plus.textContent = '+';
  ...
  posSlider.addEventListener('input', () => {
    ...
    if (window.__transformerProgress)
      window.__transformerProgress.save('section-positional');
  });
  ...
  // ===== Real Model Data (d_model=512) =====
  const realPeBtn = document.getElementById('real-pe-btn');
  const realPeContainer = document.getElementById('real-pe-container');
  let realPeLoaded = false;

  if (realPeBtn && realPeContainer) {
    realPeBtn.addEventListener('click', async () => {
      const isVisible = realPeContainer.style.display !== 'none';
      realPeContainer.style.display = isVisible ? 'none' : 'block';
      realPeBtn.textContent = isVisible
        ? '📊 실제 모델 데이터 (d=512)'
        : '📊 실제 모델 데이터 숨기기';
      if (!isVisible && !realPeLoaded) {
        try {
          const res = await fetch('./js/data/pe_values.json');
          const data = await res.json();
          realPeLoaded = true;
          realPeContainer.innerHTML = '';
          const label = document.createElement('div');
          label.className = 'real-data-label';
          label.textContent = `🔬 실제 sin/cos 공식으로 계산된 PE 값 (d_model=${data.d_model})`;
          realPeContainer.appendChild(label);
          const info = document.createElement('p');
          info.style.cssText =
            'color:#aaa;font-size:0.85rem;margin-bottom:12px;';
          info.textContent = `${data.positions}개 위치 × ${data.dims_shown}개 차원 (전체 ${data.d_model}차원 중 일부)`;
          realPeContainer.appendChild(info);
          const canvas = document.createElement('canvas');
          canvas.width = 420;
          canvas.height = 320;
          realPeContainer.appendChild(canvas);
          renderHeatmap(canvas, data.values, {
            scheme: 'diverging',
            cellW: 22,
            cellH: 16,
            labels: true,
