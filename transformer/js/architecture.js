...
export function initArchitecture() {
  ...
  function render(N) {
    container.innerHTML = '';
    const svgW = 740, blockW = 120, blockH = 36, gap = 10;
    ...
    addLabel(decX + blockW / 2, baseY, '출력 (Outputs, shifted right)', '#aaa', 12);

    baseY -= 40;

    // Input Embedding
    addBlock(encX, baseY - blockH, blockW, blockH, '#5C6BC0', 'Input Embedding', 'section-positional', '토큰을 벡터로 변환합니다');
    addArrow(encX + blockW / 2, baseY, encX + blockW / 2, baseY + 8);

    // Output Embedding
    addBlock(decX, baseY - blockH, blockW, blockH, '#5C6BC0', 'Output Embedding', 'section-positional', '출력 토큰을 벡터로 변환합니다');
    addArrow(decX + blockW / 2, baseY, decX + blockW / 2, baseY + 8);

    baseY -= blockH + 8;

    // Positional Encoding
    addBlock(encX, baseY - blockH, blockW, blockH, '#7986CB', 'Positional Enc.', 'section-positional', '위치 정보를 더합니다 (sin/cos)');
    addArrow(encX + blockW / 2, baseY, encX + blockW / 2, baseY - blockH + blockH);

    addBlock(decX, baseY - blockH, blockW, blockH, '#7986CB', 'Positional Enc.', 'section-positional', '위치 정보를 더합니다 (sin/cos)');
    addArrow(decX + blockW / 2, baseY, decX + blockW / 2, baseY - blockH + blockH);
    ...
    for (let n = 0; n < N; n++) {
      ...
      ey -= blockH;
      addBlock(encX, ey, blockW, blockH, '#AB47BC', 'Multi-Head Attn', 'section-multihead', '멀티-헤드 셀프 어텐션');
      if (n === 0) addArrow(encX + blockW / 2, ly, encX + blockW / 2, ey + blockH);
      // 2. Add & Norm
      ey -= blockH + gap;
      addBlock(encX, ey, blockW, blockH, '#EF5350', 'Add & Norm', 'section-addnorm', '잔차 연결 + 레이어 정규화');
      addArrow(encX + blockW / 2, ey + blockH + gap, encX + blockW / 2, ey + blockH);
      // 3. Feed Forward
      ey -= blockH + gap;
      addBlock(encX, ey, blockW, blockH, '#FFA726', 'Feed Forward', 'section-ffn', '2층 완전연결 네트워크');
      addArrow(encX + blockW / 2, ey + blockH + gap, encX + blockW / 2, ey + blockH);
      // 4. Add & Norm
      ey -= blockH + gap;
      addBlock(encX, ey, blockW, blockH, '#EF5350', 'Add & Norm', 'section-addnorm', '잔차 연결 + 레이어 정규화');
    ...
    for (let n = 0; n < N; n++) {
      ...
      dy -= blockH;
      addBlock(decX, dy, blockW, blockH, '#80CBC4', 'Masked Attn', 'section-decoder', '마스크드 셀프 어텐션 (미래 토큰 차단)');
      if (n === 0) addArrow(decX + blockW / 2, ly, decX + blockW / 2, dy + blockH);
      // 2. Add & Norm
      dy -= blockH + gap;
      addBlock(decX, dy, blockW, blockH, '#EF5350', 'Add & Norm', 'section-addnorm', '잔차 연결 + 레이어 정규화');
      addArrow(decX + blockW / 2, dy + blockH + gap, decX + blockW / 2, dy + blockH);
      // 3. Cross-Attention
      dy -= blockH + gap;
      addBlock(decX, dy, blockW, blockH, '#4DB6AC', 'Cross Attention', 'section-decoder', '인코더 출력을 K, V로 사용하는 어텐션');
      addArrow(decX + blockW / 2, dy + blockH + gap, decX + blockW / 2, dy + blockH);
      // 4. Add & Norm
      dy -= blockH + gap;
      addBlock(decX, dy, blockW, blockH, '#EF5350', 'Add & Norm', 'section-addnorm', '잔차 연결 + 레이어 정규화');
      ...
      // 5. Feed Forward
      dy -= blockH + gap;
      addBlock(decX, dy, blockW, blockH, '#FFA726', 'Feed Forward', 'section-ffn', '2층 완전연결 네트워크');
      addArrow(decX + blockW / 2, dy + blockH + gap, decX + blockW / 2, dy + blockH);
      // 6. Add & Norm
      dy -= blockH + gap;
      addBlock(decX, dy, blockW, blockH, '#EF5350', 'Add & Norm', 'section-addnorm', '잔차 연결 + 레이어 정규화');
    ...
    for (let n = 0; n < N; n++) {
      ...
      const crossAttnY = ly - blockH - (blockH + gap) - (blockH + gap) + blockH / 2;
      const line = document.createElementNS('http://www.w3.org/2000/svg', 'line');
    ...
    addBracket(encX - 2, encBracketTop, encBracketBottom - encBracketTop, encColor, `×${N}`, 'left');

    const decBracketTop = decTopY;
    const decBracketBottom = baseY;
    addBracket(decX + blockW + 2, decBracketTop, decBracketBottom - decBracketTop, decColor, `×${N}`, 'right');

    // ===== Arrow from decoder top to Linear =====
    const topY = decTopY - 20;
    addArrow(decX + blockW / 2, decTopY, decX + blockW / 2, topY + blockH);
    addBlock(decX, topY, blockW, blockH, '#FF7043', 'Linear', null, '선형 변환 (보캡 크기)');

    const softmaxY = topY - blockH - gap;
    addArrow(decX + blockW / 2, topY, decX + blockW / 2, softmaxY + blockH);
    addBlock(decX, softmaxY, blockW, blockH, '#FF5252', 'Softmax', null, '확률 분포로 변환');
