...
const STAGES = [
  { id: 'input', label: '입력 토큰', color: '#5C6BC0', desc: '소스 문장의 토큰을 입력합니다' },
  { id: 'embed', label: '임베딩 + PE', color: '#7986CB', desc: '토큰을 벡터로 변환하고 위치 인코딩을 더합니다' },
  { id: 'enc-self-attn', label: '인코더 Self-Attention', color: '#AB47BC', desc: 'Q=K=V, 셀프 어텐션으로 토큰 간 관계를 학습합니다' },
  { id: 'enc-addnorm1', label: 'Add & Norm', color: '#EF5350', desc: '잔차 연결 + LayerNorm' },
  { id: 'enc-ffn', label: '인코더 FFN', color: '#FFA726', desc: '각 위치별 2층 피드포워드 네트워크' },
  { id: 'enc-addnorm2', label: 'Add & Norm', color: '#EF5350', desc: '잔차 연결 + LayerNorm' },
  { id: 'enc-output', label: '인코더 출력', color: '#7E57C2', desc: '인코더의 최종 표현이 디코더로 전달됩니다' },
  { id: 'dec-input', label: '디코더 입력', color: '#5C6BC0', desc: '타겟 시퀀스(shifted right)를 입력합니다' },
  { id: 'dec-embed', label: '디코더 임베딩+PE', color: '#7986CB', desc: '타겟 토큰을 벡터로 변환하고 위치 인코딩을 더합니다' },
  { id: 'dec-masked-attn', label: '디코더 Masked Attn', color: '#80CBC4', desc: '미래 토큰을 차단하는 마스크드 셀프 어텐션' },
  { id: 'dec-addnorm1', label: 'Add & Norm', color: '#EF5350', desc: '잔차 연결 + LayerNorm' },
  { id: 'dec-cross-attn', label: 'Cross-Attention', color: '#4DB6AC', desc: 'Q=디코더, K/V=인코더 출력' },
  { id: 'dec-addnorm2', label: 'Add & Norm', color: '#EF5350', desc: '잔차 연결 + LayerNorm' },
  { id: 'dec-ffn', label: '디코더 FFN', color: '#FFA726', desc: '각 위치별 2층 피드포워드 네트워크' },
  { id: 'dec-addnorm3', label: 'Add & Norm', color: '#EF5350', desc: '잔차 연결 + LayerNorm' },
  { id: 'linear', label: 'Linear', color: '#FF7043', desc: '보캡 크기로 선형 변환' },
  { id: 'softmax', label: 'Softmax', color: '#FF5252', desc: '확률 분포로 변환' },
  { id: 'output', label: '출력 토큰', color: '#26A69A', desc: '가장 높은 확률의 토큰을 선택합니다' },
];
...
export function initDataflow() {
  ...
  function render() {
    container.innerHTML = '';
    const svgW = 900, svgH = 100;
    const stageW = 50, gapX = 6;
    ...
    STAGES.forEach((stage, i) => {
      ...
      rect.setAttribute('width', stageW); rect.setAttribute('height', 40);
      ...
      text.setAttribute('x', x + stageW / 2); text.setAttribute('y', 80);
      ...
      if (i < STAGES.length - 1) {
        ...
        arrow.setAttribute('x1', x + stageW); arrow.setAttribute('y1', 40);
        arrow.setAttribute('x2', x + stageW + gapX); arrow.setAttribute('y2', 40);
      ...
      if (i === 0) {
        ...
        g.setAttribute('x', x); g.setAttribute('y', 14);
        g.setAttribute('fill', '#7E57C2'); g.setAttribute('font-size', '9');
      ...
      if (i === 9) {
        const g = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        g.setAttribute('x', x); g.setAttribute('y', 14);
        g.setAttribute('fill', '#26A69A'); g.setAttribute('font-size', '9');
